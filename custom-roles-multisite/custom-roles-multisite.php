  ...
  private function get_franchisee_page_capabilities() {
    ...
    public function restrict_admin_menu() {
        ...
        if (current_user_can('franchise_business_consultant')) {
            $allowed_menus = [
                'index.php', 'edit.php', 'edit.php?post_type=estate-sale',
                'gf_edit_forms', 'looker-studio-dashboard', 'scribe-ai-dashboard',
            ];
    ...
    public function enforce_acf_field_permissions($field) {
      ...
      if ($field['name'] === 'iframe_embed_code' ||
          !is_admin() ||
      ...
      if (isset($permissions[$field['key']])) {
          ...
          if (!$has_permission) {
              ...
              $field['instructions'] .= ' You do not have the correct permissions to edit this.';
          }
      }
  ...
  public function prevent_unauthorized_acf_save($value, $post_id, $field) {
    ...
    public function hide_admin_notifications() {
        if (!$this->is_admin_or_super_admin()) {
            ...
            add_action('admin_menu', function() { remove_submenu_page('index.php', 'about.php'); });
            add_action('wp_dashboard_setup', function() { remove_meta_box('dashboard_primary', 'dashboard', 'side'); });
        }
    }
    ...
    public function enqueue_custom_acf_js() {
      wp_enqueue_script('custom-acf-disable', plugin_dir_url(__FILE__) . 'assets/js/custom-acf-disable.js', ['jquery', 'acf-input'], '1.0', true);
  }
    ...
    public function check_version() {
        if (get_option('custom_roles_version') != '7.4') {
            $this->create_custom_user_roles();
            $this->add_dashboard_capabilities();
            update_option('custom_roles_version', '7.4');
        }
    }
